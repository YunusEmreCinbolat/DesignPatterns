# Sepet Fiyatlandırma Sistemi Üzerinden Flyweight, Bridge ve Facade Tasarım Desenlerini Derinlemesine İnceleme
...
Bu makalede, demo bir **sepet fiyatlandırma sistemi** üzerinden üç kritik tasarım desenini ele alıyoruz:
...
Bu projede çözüm: sorumlulukları 3 pattern ile bölmek.
---
...
## 3. Bridge Pattern — Checkout ile Shipping’i Gevşek Bağlamak
...
Bu projede `DiscountFacade`, indirim hesaplama + Bridge bileşenlerini orkestre ederek **tek bir metotla** fiyatlandırma sonucu üretir:
...
### 4.1 Facade Pattern’ın Sağladığı Avantajlar
...
## 5. Controller Katmanı — UI ile Pattern Dünyasının Buluştuğu Yer
...
### 5.1 Ürünleri Listeleme (Flyweight Cache’inden)
...
### 5.2 Sepet Fiyatı Hesaplama Endpoint’i
...
## 6. Service Katmanı — Flyweight + Facade ile Akışı Birleştirmek
...
## 7. Angular Frontend — Sepet Akışını Tamamlayan Katman
...
8) Facade içinde:
    - Seçilen indirim tipine göre indirim hesaplanır.
...
### 13.1 Yeni indirim eklemek
Örneğin “1000 TL üstü 100 TL indirim” gibi bir indirim için:
- Yeni bir `Discount` sınıfı yazılır
- `resolveStrategy(...)` (indirim seçimini yapan kısım) içine yeni enum case eklenir

### 13.2 Yeni shipping kuralı eklemek
...
## 14. API – Frontend – Backend Üçlüsünün Uyumlu Çalışması
...
## 15. Örnek Bir İsteğin Uçtan Uca Görünümü
Örnek payload:
...
- İndirim hesabı için `BuyXGetYDiscount("P1", 3, 1)` seçer
- Sonra Bridge ile kargoyu hesaplayıp sonucu döner

---

## 16. Sistemin Güvenilirlik Analizi

### 16.1 Veri Tutarlılığı
...
### 16.2 Bakım Kolaylığı
İndirim, kargo ve orkestrasyon katmanları ayrıldığı için değişiklikler lokal kalır.

---

## 17. Bu Mimari Üzerine Potansiyel Geliştirmeler
- `resolveStrategy(...)` indirim seçimini Spring üzerinden “registry” mantığına taşımak
...
## 18. Kapanış
Bu sepet fiyatlandırma projesi; Flyweight, Bridge ve Facade Pattern’lerinin **aynı problem üzerinde** nasıl birlikte çalıştığını net şekilde gösterir.
